...
//Methods are not copied because they belong to the class definition and are shared by all objects.
module tb;
...
h1.display();
	h2 = h1.copy();  //have to use this syntax for deep copy -->new will work if the top class(here "ins_class") have only variable and no class handles(that will be shallow copy)
